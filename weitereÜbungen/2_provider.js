...
// Your code here!
const mainUrl = process.env.INFURA_MAINNET_API_URL;
mainkey = `${mainUrl}${process.env.INFURA_KEY}`;

const mainprovider = new ethers.providers.JsonRpcProvider(mainkey);
...
const testUrl = process.env.INFURA_GOERLI_API_URL;
const testkey = `${testUrl}${process.env.INFURA_KEY}`;
const testprovider = new ethers.providers.JsonRpcProvider(testkey);

const blockdif = async () => {
    const mainblock = await mainprovider.getBlock();
    const testblock = await testprovider.getBlock();
    console.log(`blocknumber mainnet: ${mainblock.number}`);
    console.log(`blocknumber testnet: ${testblock.number}`);
    console.log(`blockdif: ${mainblock.number-testblock.number}`);
}
blockdif();
